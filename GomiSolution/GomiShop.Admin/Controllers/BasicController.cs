...
using GomiShop.Admin.Infrastructure;
using GomiShop.Admin.Models;
                        ...
                        if (file.IsImage())
                        {
                            if (file != null && file.ContentLength > 0)
                            {
                                if (!string.IsNullOrEmpty(file.FileName.Replace("\"", string.Empty)))
                                {
                                    ...
                                    pictureList.Add(FileHelpers.SaveImageByWeb(file, imageName, pathTemp, string.Empty, 1200));
                                }
                            }
                        }
                    }
                        ...
                        if (file.IsImage())
                        {
                            if (file != null && file.ContentLength > 0)
                            {
                                if (!string.IsNullOrEmpty(file.FileName.Replace("\"", string.Empty)))
                                {
                                    ...
                                    var newFile = FileHelpers.SaveImageByWeb(file, imageName, pathTemp, string.Empty, 1200);
                                    // save image into folder Temp
        ...
        public void CreateSessionCkFinder(bool isCreateSession, string folderSave)
        {
            if (Request.IsAuthenticated)
            {
                if (isCreateSession)
                {
                    ...
                    Session["UserRole"] = user.FindFirstValue("AccountRole") + AppSettings.Delimiter + folderSave;
                }
                else
                {
